...
/*   Updated: 2025/10/29 15:19:38 by vahdekiv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	forks_init(t_philo *philo, t_fork forks, i)
{
		philo->first_fork->fork_id = i;
		printf("philo %i first fork id = %i\n", i, philo->first_fork->fork_id);
		philo->second_fork->fork_id = (i + 1) % table->num_of_philos;
